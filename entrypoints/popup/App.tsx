import { ExternalLink, Loader2, Settings, Sparkles } from 'lucide-react';
    ...
    <main className="w-80 bg-white p-4 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
      <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        ...
        <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
          {status
            ? isReady
              ? `Using ${status.provider} with ${status.model}. Open any web page and use the floating chat button.`
        ...
        <button
          ...
          className={`mt-4 inline-flex w-full items-center justify-center gap-2 rounded-md px-3 py-2 text-sm font-semibold shadow-sm transition focus:outline-none focus:ring-2 focus:ring-sky-500 ${
            isReady
